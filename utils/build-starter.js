...
const generateNewApp = require('siapi-generate-new');
...
module.exports = async function buildStarter(programArgs, program) {
  ...
  const tmpDir = await fse.mkdtemp(join(os.tmpdir(), 'siapi-'));
  ...
  // Set command options for Siapi app
  const generateSiapiAppOptions = {
    ...program,
    starter: fullUrl,
  ...
  // Create siapi app using the template
  await generateNewApp(join(rootPath, 'backend'), generateSiapiAppOptions);

  // Install frontend dependencies
  console.log(`Creating Siapi starter frontend at ${chalk.green(frontendPath)}.`);
